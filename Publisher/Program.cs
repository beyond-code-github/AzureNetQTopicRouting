    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var bus = AzureBusFactory.CreateBus(connectionstring))
            {
                ...
                {
                    bus.Publish(new TextMessage { Text = string.Format("Testing topic one: '{0}'", input) }, "topic.one");
                    bus.Publish(new TextMessage { Text = string.Format("Testing topic two: '{0}'", input) }, "topic.two");
                }
            }
        }
    }
